...
function az_upload {
    PATH_TO_BACKUP=$POD_NAMESPACE"/"$SCOPE"/"$PG_VERSION"/logical_backups/"$(date +%s).sql.gz
...
function aws_delete_outdated {
    ...
    prefix=$LOGICAL_BACKUP_S3_BUCKET_PREFIX"/"$POD_NAMESPACE"/"$SCOPE"/"$PG_VERSION"/logical_backups/"
...
function aws_upload {
    ...
    PATH_TO_BACKUP=s3://$LOGICAL_BACKUP_S3_BUCKET"/"$LOGICAL_BACKUP_S3_BUCKET_PREFIX"/"$POD_NAMESPACE"/"$SCOPE"/"$PG_VERSION"/logical_backups/"$(date +%s).sql.gz
...
function gcs_upload {
    PATH_TO_BACKUP=gs://$LOGICAL_BACKUP_S3_BUCKET"/"$LOGICAL_BACKUP_S3_BUCKET_PREFIX"/"$POD_NAMESPACE"/"$SCOPE"/"$PG_VERSION"/logical_backups/"$(date +%s).sql.gz
